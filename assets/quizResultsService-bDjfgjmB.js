import{l as r,a3 as c}from"./index-qLdPM7i1.js";class z{async saveQuizResult(e){try{r.info("Saving quiz result to database","QuizResultsService",{quizId:e.quizId,score:e.score,totalQuestions:e.totalQuestions,userId:e.userId});const{data:i,error:u}=await c.from("quiz_results").insert([{user_id:e.userId||null,quiz_id:e.quizId,score:e.score,total_questions:e.totalQuestions,answers:e.answers,time_taken:e.timeTaken||null,created_at:new Date().toISOString()}]).select().single();return u?(r.error("Failed to save quiz result","QuizResultsService",{error:u.message,code:u.code}),null):(r.info("Quiz result saved successfully","QuizResultsService",{resultId:i.id,quizId:e.quizId}),i.id)}catch(i){return r.error("Error saving quiz result","QuizResultsService",{},i),null}}async getQuizHistory(e){try{r.info("Fetching quiz history","QuizResultsService",{quizId:e.quizId,userId:e.userId,limit:e.limit});let i=c.from("quiz_results").select(`
          id,
          user_id,
          quiz_id,
          score,
