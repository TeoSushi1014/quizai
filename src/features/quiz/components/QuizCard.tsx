...
interface QuizCardProps {
  ...
  // onTakeQuiz and onPracticeQuiz are no longer needed as props, navigation handled internally
...
const QuizCard: React.FC<QuizCardProps> = ({ quiz, onDelete, onEditQuiz }) => {
  ...
  return (
    <>
      <Card 
          className={`flex flex-col justify-between transition-all duration-200 ease-out group relative overflow-hidden !p-0 !rounded-2xl h-full`}
          useGlassEffect={true} 
      >
        <div className="p-5 sm:p-6 flex-grow pb-4">
          <h3 
              className="text-lg sm:text-xl font-semibold text-slate-50 mb-3 group-hover:text-sky-300 transition-colors duration-150 line-clamp-2" 
              title={quiz.title}
          >
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-2 w-full sm:w-auto justify-center sm:justify-start">
            ...
            <Tooltip content={t('settings')} wrapperClassName="inline-flex flex-grow xs:flex-grow-0">
                <Button
                    size="sm"
                    variant="outline"
                    onClick={openAttemptSettingsModal} // Only settings button opens modal
                    className="w-full py-2 px-4 rounded-lg border-slate-500/70 hover:border-sky-400"
      ...
      {isAttemptSettingsModalOpen && (
        <Modal
          ...
          onClose={() => setIsAttemptSettingsModalOpen(false)}
          title={
          ...
          footerContent={
            <div className="flex flex-col sm:flex-row justify-end gap-3.5">
              <Button variant="secondary" onClick={() => setIsAttemptSettingsModalOpen(false)} size="md" className="w-full sm:w-auto">
                {t('cancel')}
              </Button>
