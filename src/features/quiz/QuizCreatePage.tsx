...
const QuizCreatePage: React.FC = () => {
  ...
  const renderStepContent = () => {
    switch (step) {
      case 1: return ( <Card className="max-w-xl mx-auto shadow-2xl !rounded-2xl" useGlassEffect> <div className="space-y-8"> <Dropzone onFileUpload={handleFileUpload} acceptedFileTypes=".pdf,.txt,.docx,.jpg,.png,.jpeg" maxFileSizeMB={10} label={<p className="text-base font-semibold text-[var(--color-text-primary)] mb-2">{t('step1UploadOrDrag')}</p>} icon={<img src="https://img.icons8.com/?size=256&id=hwKgsZN5Is2H&format=png" alt={t('upload')} className="w-12 h-12 sm:w-14 sm:h-14 mb-3.5 text-[var(--color-text-muted)] group-hover:text-[var(--color-primary-accent)] transition-all var(--duration-fast) var(--ease-ios) group-hover:scale-105" />} isLoading={isProcessingFile} currentFile={uploadedFile} /> {isProcessingFile && progress > 0 && <ProgressBar progress={progress} label={t('step1ProcessingFile')} className="mt-4 animate-fadeIn" />} <div className="relative flex py-1 items-center"> <div className="flex-grow border-t border-[var(--color-border-default)]"></div> <span className="flex-shrink mx-4 text-[var(--color-text-muted)] text-xs font-medium uppercase tracking-wider">{t('step1Or')}</span> <div className="flex-grow border-t border-[var(--color-border-default)]"></div> </div> <Textarea label={<p className="text-base font-semibold text-[var(--color-text-primary)]">{t('step1PasteTextLabel')}</p>} value={pastedText} onChange={(e) => setPastedText(e.target.value)} placeholder={t('step1PasteTextPlaceholder')} rows={6} className="min-h-[150px] text-sm paste-text-area" /> <Button onClick={handlePasteText} disabled={!pastedText.trim() || isProcessingFile} fullWidth size="lg" variant="secondary" className="py-3 shadow-lg"> {t('step1UsePastedText')} </Button> {processingError && !isProcessingFile && (<div role="alert" className={`p-3.5 bg-red-500/20 border border-red-500/50 rounded-lg text-sm text-red-300 text-center shadow-md animate-fadeIn`}>{processingError}</div>)} </div> </Card> );
      case 2:
