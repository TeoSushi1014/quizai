...
import QuizCard from './components/QuizCard'; // Import the shared QuizCard
...
const FeedbackSection: React.FC = () => {
  ...
  return (
    <section className="py-12 md:py-16">
      <Card className="max-w-2xl mx-auto shadow-2xl !rounded-2xl glass-effect !border-slate-700/40" useGlassEffect>
        <div className="text-center mb-6 sm:mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-50 mb-2.5 sm:mb-3">{t('feedbackSectionTitle')}</h2>
          <p className="text-slate-300/80 text-sm sm:text-base max-w-xl mx-auto">{t('feedbackSectionSubtitle')}</p>
        </div>
...
const HomePage: React.FC = () => {
  const { quizzes, currentUser, setCurrentView, deleteQuiz } = useAppContext(); // quizzes is now pre-filtered
  const { t } = useTranslation();
  ...
  // The `quizzes` from context is already filtered based on auth state.
  // If logged in, it's their quizzes. If logged out, it's anonymous quizzes.
  if (quizzes.length === 0) {
    return (
      <div className="space-y-12 md:space-y-16">
        <section className={`relative text-center py-20 md:py-28 rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl
          bg-gradient-to-br from-sky-600/20 via-slate-800/50 to-purple-600/20
          border border-slate-700/40`}>
          <div className="relative z-10 container mx-auto px-4">
            <h1 className="text-3xl sm:text-4xl md:text-5xl xl:text-6xl font-extrabold text-slate-50 leading-tight mb-6 sm:mb-8">
            ...
            <p className="text-sm sm:text-base md:text-lg text-slate-300/80 max-w-xl md:max-w-2xl xl:max-w-3xl mx-auto mb-10 sm:mb-12">
              {t('heroSubtitle')}
            </p>
            <Button
              size="md"
              ...
              className="shadow-2xl hover:shadow-sky-400/50 transform hover:scale-105 focus:ring-offset-transparent py-3 px-8 sm:py-3.5 sm:px-10 text-sm sm:text-base rounded-xl"
            >
              {heroButtonText}
            </Button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-6">
            <div>
              ...
              <p className="text-slate-300 text-sm sm:text-base">
                {t('homeStatsQuizzes', { count: quizzes.length })} {/* quizzes.length is now correctly filtered */}
              </p>
            </div>
            <Button
              variant="primary"
              ...
              leftIcon={<PlusIcon className="w-5 h-5" strokeWidth={2.5} />}
              className="shadow-xl hover:shadow-sky-400/50 transform hover:scale-105 py-3 px-7 rounded-xl w-full sm:w-auto flex-shrink-0"
            >
